...
import {alliances,byId,Alliance} from './data';
import './style.css';
import './news.css';
...
function Button({
  ...
  return to
    ? <Link className={c} to={to}>{children}<ArrowRight size={18}/></Link>
...
function Footer(){
  return <footer>
    <div className="footer-mark">
      ♜ <b>K1391</b>
      <span>KINGSHOT KINGDOM 1391</span>
    </div>

    <div>
      {nav.slice(0,5).map(([p,n])=>(
        <Link key={p} to={p}>{n}</Link>
      ))}
    </div>

    <Link className="discord-mini" to="/community">
      <MessageCircle/> Join Discord
    </Link>

    <p>
      Alliance information and schedules may change.
      Please confirm current details with alliance leadership.
    </p>
  </footer>
}
...
function AllianceBadge({a}:{a:Alliance}){
  return <div
    className="badge"
...
function EventRows({a}:{a:Alliance}){
  return <div className="events">
    {[
      ...
      <div key={String(n)}>
        <span>{icon}</span>
        <b>{n}</b>
        <em>{(t as string[]).join(' / ')}</em>
      </div>
    ))}
  </div>
}

function AllianceCard({a}:{a:Alliance}){
  return <article
  ...
  >
    ...
    <div className="card-main">
      <h2>{a.id} <small>ALLIANCE</small></h2>
      <p>{a.description}</p>
      <EventRows a={a}/>
  ...
  return <>
    <section className="hero">
      ...
      <div className="hero-sign">
        <Sign
          eyebrow="KINGSHOT KINGDOM 1391"
          title="FIND YOUR\nFOREVER HOME"
          copy="in K1391"
        />

        <p className="hero-copy">
          A friendly kingdom for active players,<br/>
    ...
    <section className="welcome wrap">
      <div className="parchment pinned">
        ...
        <h2>WELCOME TO KINGDOM 1391</h2>
        <p className="lead">Looking for a new home?</p>
        <p>
          Kingdom 1391 welcomes players looking for an active,
          organised and friendly kingdom. Explore our alliances,
          compare event schedules, meet alliance representatives
          and find the place that fits your playstyle.
        </p>
    ...
    <section className="home-quest wrap">
      <div>
        ...
        <h2>A kingdom worth<br/>calling home.</h2>
        <p>
          Meet allies, discover battle schedules,
          and begin your transfer journey with confidence.
        </p>
...
function Alliances(){
  ...
  const showing=
    filter==='ALL'
      ? alliances
      : alliances.filter(a=>a.id===filter);
  ...
  return <>
    ...
    <section className="alliance-page wrap">
      ...
      <p className="time-note">
        ⏱ All times listed are Kingshot time (UTC).
        Player IDs are provided in case alliance players change their usernames.
      </p>
    </section>
  </>
}
...
function Detail(){
  ...
  return <>
    ...
    >
      ...
      <div>
        <span>THE ALLIANCE OF K1391</span>
        <h1>{a.id} ALLIANCE</h1>
        <p>{a.description}</p>
      </div>
    </section>
    ...
    <section className="detail wrap">
      ...
      <div className="contact-board">
        ...
        {a.contacts.map(c=>(
          <div className="contact" key={c.playerId}>
            <span>✉</span>
          ...
          <div className="event-board" key={key}>
            ...
            {alliances.map(a=>(
              <Link to={`/alliances/${a.id}`} key={a.id}>
                <AllianceBadge a={a}/>
                <b>{a.id}</b>
                <em>{a.events[key].map(conv).join(' / ')}</em>
              </Link>
            ))}
          </div>
        ))}
...
function Records(){
  ...
  return <>
    <PageHero
      title="THE KVK\nRECORD HALL"
      subtitle="The campaigns that have shaped Kingdom 1391."
    />
      ...
      <div className="record-board">
        ...
        {matches.map(([kvk,opponent,prep,battle])=>(
          ...
          >
            ...
            <span className={`result ${battle.toLowerCase()}`}>
              {battle}
            </span>
          </div>
        ))}
        ...
        <p className="record-note">
          Records shown reflect the supplied Kingdom 1391 KVK history.
        </p>
      </div>
    </section>
  </>
...
function Apply(){
  ...
  const submit=async(e:React.FormEvent)=>{
    ...
    const lastSent=Number(
      localStorage.getItem('k1391-enquiry-sent-at')
    );
    ...
    try{
      ...
      setErr(
        'The messenger could not reach the kingdom ledger. Please try again shortly.'
      );
    }finally{
      setSending(false);
    }
  ...
  if(sent)
    return <section className="success wrap">
      ...
      <div className="parchment">
        ...
        <p>
          ...
          An alliance representative will contact you with the next steps.
        </p>
...
function Kingdom(){
  const news=[
    [
      '⚔',
      'KVK CAMPAIGN RECORD',
      'Kingdom 1391 stands at 7–0 in Preparation Phase and 5–2 in Battle Phase across the recorded campaigns.',
      'VIEW KVK RECORDS',
      '/records'
    ],
    [
      '🕐',
      'ALLIANCE EVENT SCHEDULES',
      'Compare Bear, Vikings, Swordland and 3Alliance times to find the alliance rhythm that fits you.',
      'VIEW SCHEDULE',
      '/schedule'
    ],
    [
      '🛡',
      'ALLIANCE HALL OPEN',
      'Explore the five alliance halls, meet their listed representatives, and ask about joining.',
      'EXPLORE ALLIANCES',
      '/alliances'
    ],
    [
      '💬',
      'COMMUNITY GATE',
      'Ask questions, meet the kingdom, and watch for the official Discord invite when it is available.',
      'VISIT COMMUNITY',
      '/community'
    ]
  ];
    ...
    <section className="news wrap">
      <div className="news-banner parchment">
        ...
        <div>
          ...
          <p>
            Keep up with alliance activity, event schedules,
            KVK history and community announcements in one place.
          </p>
        </div>
      ...
      <div className="news-grid">
        {news.map(([icon,title,copy,cta,to],i)=>(
          <article
            className={`news-card news-${i}`}
            key={title}
          >
      ...
      <p className="news-note">
        New kingdom announcements can be added here as they are confirmed by leadership.
      </p>
    </section>
  </>
}
    ...
    <section className="community wrap">
      <div className="discord-panel">
        ...
        <div>
          ...
          <p>
            Join the conversation for kingdom announcements,
            alliance contacts, transfer discussions, and community chat.
          </p>

          <a
            href="https://discord.com/channels/1461962057346846762/1461962057938108439/1546087990416117813"
            target="_blank"
            rel="noreferrer"
...
function FAQ(){
  const qs=[
    [
      'What is Kingdom 1391?',
      'A player community where you can explore alliance options, event schedules and transfer contacts.'
    ],
    [
      'How does kingdom transfer work?',
      'Transfer details can change, so review the checklist and confirm the latest requirements with alliance leadership.'
    ],
    [
      'Which alliances are recruiting?',
      'The available alliance halls listed here are the current sample information. Contact a leader to confirm availability.'
    ],
    [
      'What are the alliance event times?',
      'Each alliance page and the schedule board show event times in Kingshot time (UTC).'
    ],
    [
      'How do I contact an alliance leader?',
      'Open an alliance hall to find the listed contact name and player ID.'
    ],
    [
      "Can I apply if I don't know which alliance I want?",
      'Absolutely. Select “Not Sure Yet” on the application scroll.'
    ],
    [
      'What timezone are schedules in?',
      'Schedules default to Kingshot time (UTC); use the schedule switch to see browser-local conversions.'
    ],
    [
      'How long does it take to receive a response?',
      'Response times vary. A leader will share the next steps when they can.'
    ]
  ];
...
function NotFound(){
  return <section className="not-found wrap">
    <div className="parchment">
      ...
      <p>We couldn't find that alliance hall.</p>
